      ...
      CKEDITOR.instances.secondoption.on('key', function (e) {
        ...
        setTimeout(function () {
          ...
          if (resultado != "") {
            ...
            $.get(url + "word/find/" + resultado, function (data) {
              let word = word.replace(resultado, data.word.description).replace("*", " ");
              word = word.replace("*", " ");
...
function setupDatatables() {
  console.log("Table ini", $.fn.dataTable.isDataTable(".example2"));
  ...
  if (!$.fn.dataTable.isDataTable("#exampleAjax")) {
    $('#exampleAjax').DataTable({
      "ajax": url + "results.json",
      "language": {
        "lengthMenu": "_MENU_ registros por página",
        "zeroRecords": "Cargando Registros.....",
        "info": "Página _PAGE_ de _PAGES_",
        "infoEmpty": "No hay registros disponibles",
        "search": "Buscar:",
        "infoFiltered": "(filtrada a partir de  _MAX_ entradas totales)",
      ...
      "columnDefs": [{
        ...
        "render": function (data, type, row) {
          ...
          $gestion = `<button class="btn btn-primary btn-xs open_modal" 
                            model="results/" 
          ...
          return (row[6] != 3) ? $gestion + ` <a href="results/certificate/${row[0]}" class="btn btn-xs btn-danger">C</a>` : `<a href="results/certificate/${row[0]}" class="btn btn-xs btn-danger">C</a>`;
        }
      }],
...
function addParameters() {
  ...
  </select>`, `<input type="number" id="row-3-age" name="positions[]" value="${counter}">`,
  `<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>`]).draw(false);
    counter++;
  });
  $('#parameters tbody').on( 'click', 'button.close', function () {
    t
        .row( $(this).parents('tr') )
        .remove()
        .draw();
